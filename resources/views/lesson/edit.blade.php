<x-app>
    ...
    <div class="card shadow-lg p-3">
        ...
        <form action="{{ route('lesson.update', $lesson) }}" method="post" enctype="multipart/form-data" class="form">
            @csrf
            @method('put')
                ...
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('title') is-invalid @enderror" type="text" id="title"
                            name="title" required value="{{ old('title', $lesson->title) }}">
                        @error('title')
                ...
                <div class="col-md-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2-default @error('type') is-invalid @enderror"
                            ...
                            <option value="text" @selected(old('type', $lesson->type) == 'text')>Teks</option>
                            <option value="video" @selected(old('type', $lesson->type) == 'video')>Video</option>
                            <option value="pdf" @selected(old('type', $lesson->type) == 'pdf')>PDF</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3" id="file_upload_group" style="display: none;">
                        <label for="file" class="form-label">Upload File Baru (menggantikan yang lama)</label>
                    ...
                    <div class="mb-3" id="file_link_group" style="display: none;">
                        ...
                        <input class="form-control @error('file_link') is-invalid @enderror" type="text" id="file_link" name="file_link" value="{{ old('file_link', $lesson->file_path) }}">
                        @error('file_link')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('order') is-invalid @enderror" type="number" id="order"
                            name="order" required min="1" value="{{ old('order', $lesson->order) }}">
                        @error('order')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
