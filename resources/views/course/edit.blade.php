<x-app>
    ...
    <div class="card shadow-lg p-3">
        ...
        <form action="{{ route('course.update', $course) }}" method="post" enctype="multipart/form-data" class="form">
            @csrf
            @method('put')
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-3">
                    ...
                    <img src="{{ $course->thumbnail ? asset('storage/' . $course->thumbnail) : asset('niceadmin/img/noprofil.png') }}"
                        alt="Thumbnail" class="w-100 rounded mt-2" id="preview">
                </div>
                ...
                <div class="col-md-9">
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('title') is-invalid @enderror" type="text" id="title"
                            name="title" required value="{{ old('title', $course->title) }}">
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select select2-default @error('level') is-invalid @enderror"
                                    ...
                                    @foreach (['N5', 'N4', 'N3', 'N2', 'N1', 'Umum'] as $lvl)
                                        <option value="{{ $lvl }}"
                                            @selected(old('level', $course->level) == $lvl)>{{ $lvl }}</option>
                                    @endforeach
                                </select>
                                @error('level')
                                    <div class="invalid-feedback">{{ $message }}</div>
